...
echo == 启动后端和前端 ==
start "MiroworldBackend" /b cmd /c "cd /d ""%APP_DIR%"" && npm run backend > ""%LOG%.backend"" 2>&1"
start "MiroworldFrontend" /b cmd /c "cd /d ""%APP_DIR%\frontend"" && npm run dev > ""%LOG%.frontend"" 2>&1"
