...
# ------------------------------------------------------------------------------
# 醒目告知安装位置
# 便携设计：刻意「在哪执行就装哪」，方便装到 U 盘/移动硬盘随身携带。
# 因此不改变该行为，只是把目标路径显著地告诉用户，避免在意料之外的目录安装。
# 需要装到别处：先 cd 到目标目录再执行本脚本。
# ------------------------------------------------------------------------------
echo ""
echo -e "${BLUE}============================================================${NC}"
echo -e "${BLUE}  安装位置确认（便携模式：安装到当前所在目录）${NC}"
echo -e "${BLUE}============================================================${NC}"
echo -e "  当前目录：${GREEN}${PROJECT_ROOT}${NC}"
echo -e "  将安装到：${GREEN}${PROJECT_ROOT}/${TARGET_DIR}${NC}"
echo ""
echo -e "  ${YELLOW}本程序为便携设计，可装在 U 盘或移动硬盘中随身使用。${NC}"
echo -e "  ${YELLOW}若想换个位置：按 Ctrl+C 中断，cd 到目标目录后重新执行。${NC}"
echo -e "${BLUE}============================================================${NC}"
log_to_file "安装目标目录: $PROJECT_ROOT/$TARGET_DIR"

# 交互式终端给 5 秒反悔时间；管道/非交互环境（curl | bash）直接继续，不阻塞自动化
if [ -t 0 ]; then
    echo -e "  ${BLUE}5 秒后开始安装…（按 Ctrl+C 可中断）${NC}"
    sleep 5
fi
echo ""
