  ...
  <div class="world-view" :class="{ 'high-contrast': highContrast }">
    ...
    <div class="world-body">
      ...
      <WorldBiblePanel
        ...
        :detecting="detecting"
        :save-msg="saveMsg"
        :save-msg-error="saveMsgError"
        :stats="stats"
        :saved-files="savedFiles"
        :can-detect="canDetect"
        @save="handleSave"
        @detect="handleDetect"
        @bg-files-change="(files) => bgFiles = files"
        @st-files-change="(files) => stFiles = files"
      />

      <!-- 冲突检测结果（模块化组件） -->
      <WorldConflictReportPanel
        v-if="report"
        :report="report"
        :project-id="projectId"
        :conflict-sel-mode="conflictSelMode"
        :sel-conflict-ids="selConflictIds"
        :batch-conflict-busy="batchConflictBusy"
        :justifying-id="justifyingId"
        :corr-generating-id="corrGeneratingId"
        :conf-render-busy-id="confRenderBusyId"
        @toggle-sel-mode="toggleConflictSelMode"
        @batch-accept="runBatchAccept"
        @batch-dismiss="runBatchDismiss"
        @batch-clear="selConflictIds = []"
        @toggle-conflict-select="toggleConflictSelect"
        @toggle-justify="toggleJustify"
        @set-conflict-status="({ conflict, status }) => setConflictStatus(conflict, status)"
        @toggle-conflict-history="toggleConflictHistory"
        @submit-justify="submitJustify"
        @load-conflict-corrections="({ conflict, force }) => loadConflictCorrections(conflict, force)"
      ...
      <WorldSimulationPanel
        v-if="stats"
        ref="simSection"
        v-model:simGoal="simGoal"
        v-model:simSteps="simSteps"
        v-model:simTimeMode="simTimeMode"
        v-model:simStepMin="simStepMin"
        v-model:simTimeJumps="simTimeJumps"
        v-model:simUseTimeline="simUseTimeline"
        v-model:simStorySummaryLlm="simStorySummaryLlm"
        v-model:simMaxConcurrency="simMaxConcurrency"
        v-model:simStartEventId="simStartEventId"
        v-model:selectedAgentModel="selectedAgentModel"
        v-model:graphFilterChar="graphFilterChar"
        v-model:interviewPrompt="interviewPrompt"
        v-model:whatIfQuestion="whatIfQuestion"
        :sim-status="simStatus"
        :sim-progress="simProgress"
        :sim-starting="simStarting"
        :sim-msg="simMsg"
        :sim-msg-error="simMsgError"
        :sim-events="simEvents"
        :sim-quality-issues="simQualityIssues"
        :playback-index="playbackIndex"
        :playback-playing="playbackPlaying"
        :grouped-sim-events="groupedSimEvents"
        :step-summaries="stepSummaries"
        :visible-sim-groups="visibleSimGroups"
        :sim-group-limit="simGroupLimit"
        :event-graph-data="eventGraphData"
        :graph-characters="graphCharacters"
        :sim-graph-zoom="simGraphZoom"
        :graph-pos-map="graphPosMap"
        :selected-graph-event="selectedGraphEvent"
        :show-god-mode-panel="showGodModePanel"
        :characters="characters"
        :god-injecting="godInjecting"
        :god-msg="godMsg"
        :sim-ctl-msg="simCtlMsg"
        :sim-ctl-msg-error="simCtlMsgError"
        :interview-character="interviewCharacter"
        :interviewing="interviewing"
        :interview-answer="interviewAnswer"
        :interview-msg="interviewMsg"
        :interview-msg-error="interviewMsgError"
        :report-simulation-id="reportSimulationId"
        :report-simulation-label="reportSimulationLabel"
        :report-generating="reportGenerating"
        :report-text="reportText"
        :report-blocks="reportBlocks"
        :report-empty-note="reportEmptyNote"
        :world-tree="worldTree"
        :collapsed-roots="collapsedRoots"
        :meta-undo-stack="metaUndoStack"
        :sim-history="simHistory"
        :sim-batch-mode="simBatchMode"
        :selected-sim-ids="selectedSimIds"
        :deleting-sim-batch="deletingSimBatch"
        :compare-mode="compareMode"
        :compare-selected="compareSelected"
        :what-if-base-id="whatIfBaseId"
        :what-if-base-label="whatIfBaseLabel"
        :what-if-starting="whatIfStarting"
        :what-if-msg="whatIfMsg"
        :what-if-msg-error="whatIfMsgError"
        :what-if-active="whatIfActive"
        :what-if-question-asked="whatIfQuestionAsked"
        :what-if-events="whatIfEvents"
        :what-ifing="whatIfing"
        :sim-timeline-events="simTimelineEvents"
        :available-models="availableModels"
        @start-sim="handleStartSim"
        @control="handleControl"
        @playback-prev="playbackPrev"
        @playback-toggle="playbackToggle"
        @playback-next="playbackNext"
        @rollback-step="rollbackCurrentStep"
        @open-event-detail="openEventDetail"
        @open-causal-chain="openCausalChain"
        @load-more-groups="simGroupLimit += 50"
        @zoom-graph="(delta) => simGraphZoom = Math.min(2, Math.max(0.5, simGraphZoom + delta))"
        @export-graph="exportGraph"
        @select-graph-event="(id) => selectedGraphEvent = id"
        @open-director="showDirectorModal = true"
        @toggle-god-mode="showGodModePanel = !showGodModePanel"
        @god-submit="handleGodInterventionSubmit"
        @select-character="selectCharacter"
        @send-interview="handleInterview"
        @generate-report="handleGenerateReport"
        @export-report-html="exportReportHtml"
        @undo-meta="undoLastMeta"
        @toggle-root="toggleRoot"
        @edit-meta="editWorldlineMeta"
        @load-sim="loadSimulation"
        @continue-sim="continueSimulation"
        @export-sim="exportSimulation"
        @copy-sim="copyWorldline"
        @start-whatif="startWhatIf"
        @batch-whatif="batchWhatIf"
        @delete-sim="confirmDeleteSimulation"
        @rerun-branch="rerunBranchWithSettings"
        @toggle-batch-mode="toggleSimBatchMode"
        @toggle-select-all="toggleSelectAllSims"
        @run-batch-delete="runBatchDeleteSimulations"
        @export-all="exportAllWorldlines"
        @toggle-compare-mode="toggleCompareMode"
        @toggle-select-sim="toggleSelectSim"
        @toggle-compare-select="toggleCompareSelect"
        @open-compare="openCompare"
        @merge-selected="mergeSelected"
        @open-chart-record="openChartRecord"
        @confirm-whatif="confirmWhatIf"
        @cancel-whatif="cancelWhatIf"
      />
      ...
      <!-- 事件详情抽屉（模块化组件） -->
      <WorldEventDetailModal
        :event="selectedEventDetail"
        @close="selectedEventDetail = null"
      />
      ...
      <WorldCompareModal
        ...
        @export-report="exportCompareReport"
      />
...
import WorldGraphConsole from '../components/world-setup/WorldGraphConsole.vue'
...
import WorldSimulationPanel from '../components/world-setup/WorldSimulationPanel.vue'
import WorldCompareModal from '../components/world-setup/WorldCompareModal.vue'
import WorldEventDetailModal from '../components/world-setup/WorldEventDetailModal.vue'
...
function layoutGraph(nodes, edges) {
  ...
  // 如果没有新节点且已有全部位置，直接返回已有坐标，避免每次无意义地重新耗尽 CPU
  if (!hasNewNodes && Object.keys(existingPos).length === count) {
    return pos
  }

  // 自适应轮次：节点较多时降低暴力迭代轮次，防止阻塞主线程与按钮事件
  ...
  for (let iter = 0; iter < maxIters; iter++) {
    ...
    edges.forEach(e => {
      ...
      let d = Math.sqrt(dx * dx + dy * dy) || 1
      const idealLen = 140
      const force = (d - idealLen) * 0.03 * cooling
      dx /= d
      dy /= d
      a.x += dx * force
      a.y += dy * force
