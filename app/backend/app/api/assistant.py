...
def _execute_assistant_action(project_id: str, action: str, params: dict):
    ...
    if action == "start_world_simulation":
        ...
        state = WorldSimulationService.start_simulation(
            ...
            max_concurrency=int(params["max_concurrency"]) if str(params.get("max_concurrency") or "").strip() else None,
        )
    ...
    if action == "update_worldline_meta":
        ...
        sim_id = str(params.get("simulation_id") or "").strip()
        if not sim_id:
            raise ValueError("update_worldline_meta 需要 simulation_id")
        updated = WorldSimulationService.update_simulation_meta(
            sim_id,
